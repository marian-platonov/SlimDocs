...
4. **Streamlit UI (`main()`, bottom of file)** — sidebar drives input mode (Single File / Folder / URLs), output format (`_FORMAT_OPTIONS`: md/txt/json/duckdb), and output destination; three or four tabs render results (File Processing, Statistics & Reports, Logs, and DuckDB Explorer — the last only appears when `duckdb` format is selected). All app state lives in `st.session_state`, initialized once in `_init_session()`.
   ...
   - Every run's full result dict is kept in `st.session_state.session_history` (keyed by `run_ts`), not just the latest — Statistics & Reports shows `session_history[selected_session_ts]` when set, else the latest `results`. The `st.tabs(...)` call is given `key="main_tabs", on_change="rerun"`, which makes the active tab controllable via `st.session_state["main_tabs"]`. Clicking a Logs row fires `_on_logs_row_select` (passed as `on_select` on the Logs `st.dataframe`, keyed `"logs_table"`) — this callback runs *before* the script body re-executes, so it sets `selected_session_ts` and `session_state["main_tabs"] = "📊 Statistics & Reports"` ahead of `st.tabs` being re-instantiated, which is what actually switches the visible tab. (An earlier version tried to click the tab via a `components.html` DOM script reaching into the parent document — that never reliably worked and has been replaced by this native mechanism.)
